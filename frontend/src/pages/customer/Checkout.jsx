...
export default function Checkout() {
  ...
  const handlePay = async () => {
    ...
    // 🚀 NEW FRONTEND GATEKEEPER: Instant Stock Validation
    for (const item of cartItems) {
      if (item.quantity > item.stock) {
        return alert(
          `${t("Insufficient stock for")} ${item.name}. ${t("Only")} ${item.stock} ${t("left in stock.")}`,
        );
      }
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        // SMART REDIRECT: If the backend says the address is missing, alert and redirect!
        if (
          errorData.message &&
          errorData.message.includes("Address required")
        ) {
          alert(
            t(
        ...
        // SMART ALERT: If the backend catches a stock issue (e.g. someone bought the last item 2 seconds ago)
        else if (
          errorData.message &&
      ...
      <div className="max-w-[700px] mx-auto px-4">
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-gray-200">
          ...
          <button
            onClick={handlePay}
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 rounded-2xl text-lg font-bold shadow-lg pa:shadow-xl hover:-translate-y-1 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
