...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct(product);
        // Filter related products so we only show items in stock
        setRelatedProducts(related.filter((r) => r.stock > 0));
      } catch (error) {
  ...
  const handleQuantityChange = (e) => {
    ...
    // Guardrail: Do not let customer order more than what is in stock
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-12 mt-6 mb-16">
          ...
          <div className="w-full lg:w-1/2 flex flex-col justify-center text-start">
            ...
            {/* CONDITIONAL RENDER: In Stock vs Out of Stock */}
            {product.stock > 0 ? (
              <>
                {/* LOW STOCK BANNER */}
                {product.stock <= 10 && (
                  <div className="flex items-center gap-2 text-rose-600 bg-rose-50 border border-rose-100 px-4 py-3 rounded-xl w-max mb-6">
                    <AlertCircle size={20} />
                    <span className="font-bold text-sm">
                      {t("Hurry! Only")} {product.stock} {t("left in stock.")}
                    </span>
                  </div>
                )}
              ...
              // OUT OF STOCK STATE (In case someone uses a direct link to an empty item)
              <div className="w-full md:w-auto flex items-center justify-center gap-3 bg-gray-100 text-gray-400 border-2 border-gray-200 px-10 py-5 rounded-2xl text-xl font-bold mb-10 cursor-not-allowed">
                {t("Out of Stock")}
              </div>
            )}
              ...
              relatedProducts.map((item) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {/* Related product low stock text */}
                    {item.stock <= 10 && (
                      <span className="text-[10px] font-black text-rose-500 bg-rose-50 px-1.5 py-0.5 rounded uppercase">
