...
export default function MyProducts() {
  ...
  const [filterTab, setFilterTab] = useState("all"); // NEW: State for tabs
  ...
  // NEW: Filter by both search query AND the active tab
  const filteredProducts = products.filter((product) => {
    const matchesSearch = product.name
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchesTab =
      filterTab === "all" || (filterTab === "low_stock" && product.stock <= 10);
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50/40 via-orange-50/30 to-white pb-20 py-8">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center justify-center mb-6 mt-4 text-center">
          ...
          <div className="relative w-full max-w-[600px] shadow-lg rounded-2xl mb-6">
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="w-full h-44 bg-gray-50 rounded-xl mb-4 overflow-hidden flex items-center justify-center p-2 relative">
                  ...
                  {/* DYNAMIC STOCK BADGE */}
                  <div
                    className={`absolute top-2 start-2 backdrop-blur-sm px-2.5 py-1 rounded-md text-xs font-bold shadow-sm border ${
                      product.stock <= 10
